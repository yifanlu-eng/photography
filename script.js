...
let zoom = 1;
const ZOOM_MIN = 1;   // 初始永远从 1 开始（交给舞台 flex 居中）
const ZOOM_MAX = 4;
...
// Base rendered size at zoom=1 (fits inside stage)
...
function applyTransform(skipClamp = false){
  ...
  // 注意：CSS 不再给 #lbImg 任何 translate(-50%)，避免覆盖居中逻辑
  lbImg.style.transform = `translate(${panX}px, ${panY}px) scale(${zoom})`;
  updateCursor();
}

// 只在“图片已加载 + 布局稳定”后测 baseRect
async function measureBaseRectStable(){
  if (!lbImg || !lbStage) return;

  // 等图片 decode（确保 naturalWidth 有值）
  try {
    if (lbImg.decode) await lbImg.decode();
  } catch (_) {}

  // 等两帧，确保 stage/clientWidth 和图片布局都稳定
  await new Promise(r => requestAnimationFrame(() => r()));
  await new Promise(r => requestAnimationFrame(() => r()));

  // ⚠️ 关键：测量前确保 transform 是“清零状态”
...
function createCard(category, index, src, id){
  ...
  card.innerHTML = `<img class="card-img" src="${src}" alt="${id}" loading="lazy" decoding="async">`;
...
// Resize：重新测 baseRect，然后把 pan clamp 回来
